...
categories: Lab
description: "Due to Gradescope by Sun. Jul 12 11:59 PM"
draft: false
---
...
- Go to the course organization at [github.com/sta199-su26-2](https://github.com/sta199-su26-2) organization on GitHub.
...
You should use code to support your answers to each component of this question.

::: callout-tip
Hint: The functions `distinct()` and `count()` may be helpful here.
:::
...
d\.
In the following code cell, we add one more step to the data pipelines from the previous two parts.
The new step uses slice_head(n = 1), which is designed to give the first row of the output.
Run the code cell and analyze the result for each pipeline.
You will notice the results are different.
Why?
...
How many columns does the output have?
Is the output grouped?
If so, how many groups does it have, and how many variables is it grouped on?
Articulate the difference(s) between summarize() and mutate().

::: callout-tip
Another hint: There are 2 key differences between `summarize()` and `mutate()` that your narrative should mention.
Review the help files for each if you are stuck.
:::
...
Then, briefly explain why the pivot performed in question 9 was necessary for creating the this plot.
...
- Plot should be **customized in at least one additional way** -- you could use a different than default color scale, or different than default theme (but NOT `theme_minimal()` or `theme_bw()` -- choose something more creative!)
