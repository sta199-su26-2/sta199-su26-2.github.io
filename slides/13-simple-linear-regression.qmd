...
The "we expect" and "on average" are a bit redundant, but this language precisely captures what's going on behind the scenes mathematically.
...
## Is the intercept meaningful? {.scrollable .small}
...
## Properties of least squares regression {.scrollable .smaller}
...
-   The regression line goes through the center of mass point (the coordinates corresponding to average $X$ and average $Y$ i.e., ($\bar{x}, \bar{y}$))
    ...
    -   Why?
        Under LSR (least-squares regression), $b_0 = \bar{y} - b_1~\bar{x}$; plugging in $x =\bar{x}$ to the fitted eq., $\widehat{y} = b_0 + b_1~\bar{x} = (\bar{y} - b_1~\bar{x}) + b_1~\bar{x} = \bar{y}$
...
## Goodness-of-fit {.scrollable .smaller}
